...
export default function LocationMap({ center, zoom, selected, onSelect }: LocationMapProps) {
  ...
  function useGps() {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords: { latitude: lat, longitude: lng } }) => {
        onSelect(lat, lng); setFlyTarget({ lat, lng, zoom: 17 }); setGpsLoading(false)
      },
  ...
  function applyCoords() {
    ...
    if (!p) return
    onSelect(p.lat, p.lng); setFlyTarget({ ...p, zoom: 16 }); setSearch('none'); setCoordsInput('')
  }
  ...
  async function applyPlusCode() {
    ...
    try {
      ...
      if (d.lat && d.lng) {
        onSelect(d.lat, d.lng); setFlyTarget({ lat: d.lat, lng: d.lng, zoom: 17 }); setSearch('none'); setPlusCodeInput('')
      } else { setPlusCodeError('Plus Code não encontrado.') }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={useGps} disabled={gpsLoading}
            className="flex-1 flex items-center justify-center py-2 rounded-xl text-xs font-semibold border border-gray-200 bg-white text-gray-700 hover:border-green-500 hover:text-green-700 disabled:opacity-50 transition-colors">
            {gpsLoading ? '⏳ Localizando...' : '📡 Minha localização'}
          </button>
          <button type="button" onClick={() => setSearch(s => s === 'coords' ? 'none' : 'coords')}
            className={`flex-1 flex items-center justify-center py-2 rounded-xl text-xs font-semibold border transition-colors ${
      ...
      {search === 'coords' && (
        <div className="flex gap-2">
          ...
          <button type="button" onClick={applyCoords} disabled={!parseCoords(coordsInput)}
            className="px-3 py-2 rounded-xl bg-green-700 text-white text-xs font-bold disabled:opacity-40">Ir →</button>
        </div>
      )}
      {search === 'pluscode' && (
      ...
      <div className="relative rounded-xl overflow-hidden border border-gray-200">
        <style>{`
          .location-map .leaflet-container { cursor: crosshair !important; }
          .location-map .leaflet-grab { cursor: crosshair !important; }
          .location-map .leaflet-dragging .leaflet-grab { cursor: grabbing !important; }
        `}</style>
        <div className="location-map" style={{ height: 320 }}>
          <MapContainer center={center} zoom={zoom} className="w-full h-full" zoomControl>
            <TileLayer key={satellite ? 'sat' : 'street'}
        ...
        <div className="absolute bottom-2 left-0 right-0 flex justify-center z-[1000] pointer-events-none px-2">
          <span className={`rounded-full px-3 py-1 text-xs font-medium ${selected ? 'bg-green-700/90 text-white' : 'bg-black/40 text-white'}`}>
