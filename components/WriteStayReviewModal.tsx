...
import { addStayReview, hasUserReviewedStay } from '@/lib/firestore'
...
export default function WriteStayReviewModal({ stayId, stayName, onClose, onSubmitted, zIndex = 120 }: Props) {
  const { user } = useAuth()
  ...
  async function handleSubmit() {
    ...
    setError(null)
    try {
      const already = await hasUserReviewedStay(user.uid, stayId)
      if (already) { setError('Você já avaliou esta hospedagem.'); setSubmitting(false); return }
    ...
    <div className="fixed inset-0 flex flex-col justify-end bg-black/60" style={{ zIndex }} onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 flex-shrink-0">
          <div>
            <h3 className="font-bold text-gray-900">Avaliar hospedagem</h3>
            <p className="text-xs text-gray-400 truncate max-w-[260px]">{stayName}</p>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-4 py-5 flex flex-col gap-5">
          {done ? (
            ...
            <>
              ...
              {/* Limpeza */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Como foi a limpeza?</label>
                <div className="flex gap-2">
                  {([
                    { value: 'impecavel', label: '✨ Impecável' },
                    { value: 'boa', label: '🙂 Boa' },
                    { value: 'ruim', label: '😕 Ruim' },
                  ] as const).map((opt) => (
                    <button key={opt.value} type="button" onClick={() => setCleanliness(opt.value)}
                      className={`flex-1 py-2 rounded-lg text-sm border transition-all ${cleanliness === opt.value ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-gray-600 border-gray-200'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {([
                    ...
                    <button key={opt.value} type="button" onClick={() => setService(opt.value)}
                      className={`flex-1 py-2 rounded-lg text-sm border transition-all ${service === opt.value ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-gray-600 border-gray-200'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-2">
                  {([
                    ...
                    <button key={opt.value} type="button" onClick={() => setPriceRange(opt.value)}
                      className={`flex-1 py-2 rounded-lg text-sm border transition-all ${priceRange === opt.value ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-gray-600 border-gray-200'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  placeholder="Como foi a estadia? Dicas para quem vai se hospedar..."
                  rows={4}
                  maxLength={500}
