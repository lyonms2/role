...
import { addEatReview, hasUserReviewedEat } from '@/lib/firestore'
...
export default function WriteEatReviewModal({ eatId, eatName, onClose, onSubmitted, zIndex = 120 }: Props) {
  const { user } = useAuth()
  ...
  async function handleSubmit() {
    ...
    setError(null)
    try {
      const already = await hasUserReviewedEat(user.uid, eatId)
      if (already) { setError('Você já avaliou este restaurante.'); setSubmitting(false); return }
    ...
    <div className="fixed inset-0 flex flex-col justify-end bg-black/60" style={{ zIndex }} onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 flex-shrink-0">
          <div>
            <h3 className="font-bold text-gray-900">Avaliar restaurante</h3>
            <p className="text-xs text-gray-400 truncate max-w-[260px]">{eatName}</p>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-4 py-5 flex flex-col gap-5">
          {done ? (
            ...
            <>
              ...
              {/* Qualidade da comida */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Como foi a comida?</label>
                <div className="flex gap-2">
                  {([
                    { value: 'otima', label: '😋 Excelente' },
                    { value: 'boa', label: '🙂 Boa' },
                    { value: 'ruim', label: '😕 Ruim' },
                  ] as const).map((opt) => (
                    <button key={opt.value} type="button" onClick={() => setFoodQuality(opt.value)}
                      className={`flex-1 py-2 rounded-lg text-sm border transition-all ${foodQuality === opt.value ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-gray-600 border-gray-200'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {([
                    ...
                    <button key={opt.value} type="button" onClick={() => setCrowded(opt.value)}
                      className={`flex-1 py-2 rounded-lg text-sm border transition-all ${crowded === opt.value ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-gray-600 border-gray-200'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-2">
                  {([
                    ...
                    <button key={opt.value} type="button" onClick={() => setPriceRange(opt.value)}
                      className={`flex-1 py-2 rounded-lg text-sm border transition-all ${priceRange === opt.value ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-gray-600 border-gray-200'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  placeholder="O que achou? Pratos recomendados, dicas para quem vai visitar..."
                  rows={4}
                  maxLength={500}
