...
import type { StayCategory, Stay } from '@/types'
...
export default function SugerirHospedarPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.category) { alert('Escolhe o tipo de hospedagem.'); return }
  ...
  if (status === 'success') {
    return (
      <div className="max-w-md mx-auto px-4 py-16 text-center">
        ...
        <p className="text-gray-500">A gente analisa e publica em breve. Valeu por ajudar a galera a dormir bem no rolê!</p>
        <div className="flex flex-col gap-3 mt-6">
          <button onClick={reset} className="btn-primary w-full">Indicar outra hospedagem</button>
          <Link href="/hospedar" className="text-sm text-gray-400 text-center block">← Voltar para Onde Dormir</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-8">
      <Link href="/hospedar" className="text-sm text-gray-400 mb-4 inline-block">← Onde Dormir</Link>

      <div className="text-center mb-7">
        <div className="text-4xl mb-2">🏡</div>
        <h1 className="text-2xl font-bold text-gray-900">Indicar uma hospedagem</h1>
        <p className="text-gray-500 mt-1">Conhece um lugar incrível pra descansar? Conta pra galera!</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card p-5 flex flex-col gap-4">
            ...
            {CATEGORIES.map((cat) => (
              <button key={cat} type="button" onClick={() => update('category', cat)}
                className={`px-3 py-1.5 rounded-full text-sm font-medium border transition-all ${
                  form.category === cat ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-600 border-gray-200'
                }`}>
        ...
        <div>
          ...
          <textarea required value={form.description} onChange={(e) => update('description', e.target.value)}
            rows={3} placeholder="O que tem de especial? Café da manhã incrível? Vista de tirar o fôlego? Conta tudo!"
        ...
        <div>
          ...
          <input value={form.bookingUrl} onChange={(e) => update('bookingUrl', e.target.value)}
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1.5">
            Foto da hospedagem <span className="text-gray-400 font-normal">(opcional)</span>
          </label>
          ...
          {photoPreview ? (
            ...
            <button type="button" onClick={() => fileRef.current?.click()}
              className="w-full aspect-video rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-2 text-gray-400 hover:border-green-300 hover:text-green-500 transition-colors">
        ...
        <button type="submit"
          ...
          style={{ opacity: (!form.category || uploading) ? 0.7 : 1 }}>
