            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <a
                  ...
                  className="flex items-center gap-1 text-xs font-semibold text-white bg-orange-500 hover:bg-orange-600 px-2.5 py-1.5 rounded-lg transition-colors"
                >
                  🗺️ Abrir Maps
                </a>
              </div>
              <div className="flex rounded-xl overflow-hidden border border-gray-200 mb-3">
                {([['link', '🔗 Link Maps'], ['coords', '🌐 Coordenadas'], ['pluscode', '📍 Plus Code']] as const).map(([mode, label]) => (
                  <button key={mode} type="button"
                    ...
                    className={`flex-1 py-2 text-xs font-semibold transition-colors ${locMode === mode ? 'bg-orange-500 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              {locMode === 'link' && (
                <>
                  <input value={form.mapsLink} onChange={(e) => update('mapsLink', e.target.value)}
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-400" />
                  {form.mapsLink.includes('maps') && <p className="text-xs text-orange-500 font-medium mt-1.5">📍 Link do Maps detectado</p>}
              ...
              {locMode === 'coords' && (
                <>
                  <input value={coordsInput} onChange={(e) => {
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-400 font-mono" />
                  {coordsInput && !coordsResolved && <p className="text-xs text-red-400 mt-1.5">Formato inválido. Ex: -27.1234, -48.5678</p>}
                  {coordsResolved && <p className="text-xs text-orange-500 font-medium mt-1.5">📍 Localização detectada!</p>}
                ...
                <>
                  <div className="flex gap-2">
                    <input value={plusCodeInput}
                      ...
                      className="flex-1 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-400 font-mono" />
                    <button type="button" onClick={() => resolvePlusCode(plusCodeInput)}
                      disabled={!plusCodeInput.trim() || locResolving}
                      className="px-4 py-3 rounded-xl bg-orange-500 text-white text-sm font-semibold disabled:opacity-40 whitespace-nowrap">
                      {locResolving ? '...' : 'Buscar'}
                    </button>
                  </div>
